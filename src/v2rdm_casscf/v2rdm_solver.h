...
// A 64-bit-capable stand-in for psi4's Vector, used only for the three SDP
// working vectors x/c/b. psi4::Vector's dimension is a 32-bit int, so allocating
// the constraint vector b of length n_dual_ overflows to a negative int (->
// std::length_error / "vector::_M_default_append") once n_dual_ > INT_MAX
// (~amo>=118 for C1). BigVector holds a std::vector<double> sized by size_t and
// exposes the same pointer()/zero() API the SDP code uses (all access is via
// ->pointer()/->zero(), verified across the module).
class BigVector {
  std::vector<double> data_;

public:
  BigVector(const std::string & /*name*/, long int n)
      : data_(static_cast<size_t>(n), 0.0) {}
  double *pointer() { return data_.data(); }
  void zero() {
    for (double &v : data_)
      v = 0.0;
  }
  long int dim() const { return static_cast<long int>(data_.size()); }
};
typedef std::shared_ptr<BigVector> SharedBigVector;
...
public:
  ...
  v2RDMSolver(int nalpha, int nbeta, int nmo, std::vector<double> h,
  ...
  /// Direct sparse A emitter for GPU-ADMM startup.
  bool build_sparse_A_dqg(std::vector<int> &rows, std::vector<int> &cols,
                          std::vector<double> &vals);

  /// Compact metadata for the GPU-ADMM DQG/C1 matrix-free operator.
  bool build_dqg_matrix_free_meta(std::vector<int> &int_meta,
  ...
  /// return spin-free one-particle density matrix as shared matrix for python
  /// API
  std::shared_ptr<Matrix> get_opdm();

  /// return spin-free two-particle density matrix as shared matrix for python
  /// API
  std::shared_ptr<Matrix> get_tpdm();

  /// return subset of orbitals for python API
  /**!
   * Similar to wavefunction.Ca_subset(); however, this version knows about all
   * of the CI subspaces in the SO basis. We stick to the definitions used by
   * DETCI for now.
   * @param  orbital_name fzc, drc, docc, act, ras1, ras2, ras3, ras4, pop, vir,
   * fzv, drv, or all
  ...
  /**!
   * Similar to wavefunction.Ca_subset(); however, this version knows about all
   * of the CI subspaces in the SO basis. We stick to the MCSCF definitions for
   * now.
   * @param  orbital_name FZC, DRC, DOCC, ACT, RAS1, RAS2, RAS3, RAS4, POP, VIR,
   * FZV, DRV, or ALL
  ...
  double sdp_primal_error_;
  double sdp_dual_error_;
...
protected:
  ...
  /// returns matrix of one-electron integrals associated with external
  /// hamiltonian
  ...
  // 64-bit: this is the running CONSTRAINT (dual) index used to fill b/A, and the
  // dual dimension n_dual_ exceeds INT_MAX at large active spaces (~amo>=118, C1).
  static thread_local long int offset;
  ...
  // double Generalized_Pauli_Au_term(double ** orbs,double * u,int * offa, int
  // * offb,int index); void Generalized_Pauli_ATu_term(double val, double **
  // orbs,double * A,int * offa, int * offb,int index);

  double Generalized_Pauli_Au_term(double **orbs, double *u, int ***map_a,
                                   int ***map_b, int index, double rdm_nrm,
                                   int ***sign_a, int ***sign_b);
  void Generalized_Pauli_ATu_term(int ***sign_a, int ***sign_b, double val,
                                  double **orbs, double *A, int ***map_a,
                                  int ***map_b, int index);

  double GP_N_8_Au(int &off, double *u, double **orbs, double *eigvals, int d1,
                   int d2, int d3, int d4, int d5, int d6, int d7, int d8);
  void GP_N_8_ATu(int ***sign_a, int ***sign_b, double rdm_nrm, double dum,
                  int &off, double *A, int ***map_a, int ***map_b,
                  double **orbs, int d1, int d2, int d3, int d4, int d5, int d6,
                  int d7, int d8);

  double GP_N_10_Au(int &off, double *u, double **orbs, double *eigvals, int d1,
                    int d2, int d3, int d4, int d5, int d6, int d7, int d8,
                    int d9, int d10);
  void GP_N_10_ATu(int ***sign_a, int ***sign_b, double rdm_nrm, double dum,
                   int &off, double *A, int ***map_a, int ***map_b,
  ...
  SharedBigVector c; // 1ei and 2ei of bpsdp
  SharedBigVector b; // constraint vector
  SharedBigVector x; // primal solution

  /// extended koopman's theorem
  void ExtendedKoopmans();
  void EKTEigensolver(std::shared_ptr<Matrix> V, std::shared_ptr<Matrix> D,
                      std::shared_ptr<Vector> epsilon, bool use_dggev,
                      std::string spin);

  /// compute natural orbitals and transform OPDM and TPDM to natural orbital
  /// basis
  ...
  // gidofalvi -- modified the type of tei_full_dim_ so that it is correct for
  // large bases
  ...
  /// write Qmo_ to a companion binary file alongside the checkpoint
  void WriteQmoFile();

  /// read Qmo_ from companion binary file; returns true if successful
  bool ReadQmoFromFile();

  /// default durable checkpoint filename
  std::string CheckpointFilename() const;

  /// filename for the Qmo companion binary file
  std::string QmoFilename() const;

  /// copy restart checkpoint into Psi4 scratch if a durable path was supplied
  void ImportCheckpointFileIfRequested();

  /// copy Psi4 scratch checkpoint to durable path
  void ExportCheckpointFile();

  bool checkpoint_file_imported_ = false;
  ...
  void EnergyByComponent(double &kinetic, double &potential,
